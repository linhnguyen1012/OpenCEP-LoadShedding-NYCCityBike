...
class UnsortedPatternMatchStorage(PatternMatchStorage):
    ...
    def add(self, pm: PatternMatch):
        ...
        # Apply load shedding if enabled
        if DefaultConfig.LOAD_SHEDDING_ENABLED:
    ...
    def _apply_load_shedding(self):
        """
        Applies the configured load shedding strategy if the number of partial matches exceeds the threshold.
        """
        max_size = DefaultConfig.MAX_PARTIAL_MATCHES
        if len(self._partial_matches) <= max_size:
            return
        
        strategy = DefaultConfig.LOAD_SHEDDING_STRATEGY
        if strategy == "simple":
            self._shed_simple(max_size)
        elif strategy == "efficient":
            self._shed_efficient(max_size)
        elif strategy == "fifo":
            self._shed_fifo(max_size)
        elif strategy == "batched":
            self._shed_batched(max_size)
        elif strategy == "random":
            self._shed_random(max_size)
        elif strategy == "priority":
            self._shed_priority(max_size)
        else:
            self._shed_simple(max_size)
    
    def _shed_simple(self, max_size):
        """
        Simple load shedding: remove oldest partial matches to reach max_size.
        """
    ...
    def _shed_fifo(self, max_size):
        """
        FIFO load shedding: remove the oldest partial matches when storage capacity is exceeded.
    ...
    def _shed_efficient(self, max_size):
        """
        Efficient load shedding: implements adaptive target sizing through aggressive batch removal
        to reduce shedding frequency.
        """
        ...
        self._partial_matches.sort(key=lambda pm: pm.first_timestamp)
        del self._partial_matches[:excess]
    
    def _shed_batched(self, max_size):
        """
        Batched load shedding: implements hysteresis mechanism with 20% tolerance and 25% reduction
        to minimize shedding frequency through delayed activation.
        """
        current_size = len(self._partial_matches)
    ...
    def _shed_random(self, max_size):
        """
        Random load shedding: applies uniform probability distribution for partial match removal,
        avoiding temporal bias but potentially sacrificing completion likelihood.
        """
        import random
        excess = len(self._partial_matches) - max_size
        if excess <= 0:
            return
        indices_to_remove = set(random.sample(range(len(self._partial_matches)), excess))
        self._partial_matches = [pm for i, pm in enumerate(self._partial_matches) 
                                 if i not in indices_to_remove]
    
    def _shed_priority(self, max_size):
        """
        Priority-based load shedding: removes partial matches with lowest probability first.
        """
        excess = len(self._partial_matches) - max_size
        if excess <= 0:
            return
        # Sort by probability (ascending) and remove the lowest probability matches
        self._partial_matches.sort(key=lambda pm: pm.probability)
        del self._partial_matches[:excess]
